...
async function cargarGaleria(filtro = 'todos') {
    ...
    try {
        // 1. IMPORTANTE: Destruir la instancia y limpiar el HTML totalmente
        if (swiperInstance) {
            swiperInstance.destroy(true, true);
            swiperInstance = null;
        }
        
        box.innerHTML = '<div class="swiper-slide text-center py-10">Cargando...</div>';
        ...
        // 2. Reinicializar Swiper solo si hay contenido
        swiperInstance = new Swiper(".mySwiper", {
            slidesPerView: 1,
            spaceBetween: 20,
            loop: (html !== ''), 
            observer: true,            // Ayuda a que Swiper detecte cambios
            observeParents: true,      // Ayuda a que Swiper detecte cambios
...
// 3. Vincular la función al objeto window para asegurar que los botones la vean
window.filtrarTrabajos = function(categoria) {
    const seccionTrabajos = document.getElementById('trabajos');
    if (seccionTrabajos) {
        seccionTrabajos.scrollIntoView({ behavior: 'smooth' });
    }
